            ...
            <form action="{{ route('checkout.placeOrder') }}" method="POST">
                ...
                <div class="col-md-5 order-details">
                    ...
                    <div class="payment-method">
                        ...
                        <div class="input-radio">
                            <input type="radio" name="payment_method" id="payment-3" value="momo_zalopay">
                            <label for="payment-3">
                                <span></span>
...
{{-- PHIÊN BẢN SCRIPT HOÀN CHỈNH, TỰ CHỨA MỌI THỨ VÀ ĐẶT TẠI ĐÂY --}}
@push('scripts')
<script>
    $(document).ready(function() {

        function generateQRCode() {
			const YOUR_BANK_BIN = "970422";
    const YOUR_ACCOUNT_NUMBER = "0363489746";
    const description = "Thanh toan don hang";
    const amountText = $('#checkout-total').text().replace(/[^0-9]/g, '');
    const amount = amountText || '0';
    
    // Thêm hai dòng này vào đây:
    const BANK_SHORT_NAME = "MB"; // Ví dụ, nếu bạn dùng MBBank. Cần xác nhận mã này.
    const TEMPLATE_NAME = "compact"; // Hoặc "compact2" hoặc "qr_only" tùy thích

    const apiUrl = `https://img.vietqr.io/image/${BANK_SHORT_NAME}-${YOUR_ACCOUNT_NUMBER}-${TEMPLATE_NAME}.jpg`;


	console.log("URL của ảnh QR là:", apiUrl);
    const qrImageElement = $('#qr-image');

    if (qrImageElement.length) {
        // THÊM LẠI DÒNG LỆNH QUAN TRỌNG NÀY:
        // Nó đảm bảo thẻ div cha (class="caption") chứa QR được hiện ra
        qrImageElement.closest('.caption').show();

        // Gán ảnh và style để nó hiện ra
